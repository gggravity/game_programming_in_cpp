...
// Update paddle position based on direction
    ...
    else if (mBallPos.x <= 0.0f) // Did the ball go off the screen? (if so, end game)
      {
        mIsRunning = false;
      }
    else if (mBallPos.x >= ( 1024.0f - thickness ) && mBallVel.x > 0.0f) // Did the ball collide with the right wall?
      {
        mBallVel.x *= -1.0f;
      }
